...
source(here("scripts", "venv_setup", "utils.R"))
...
process_spacy_installation_conda <- function(conda, python_version, prompt,
  ...
  if (nrow(conda_env) == 1) {
    ...
    python_version_reticulate <- ifelse(is.null(python_version), "python=3.6",
  ...
  py_packages <- here("scripts", "venv_setup", "requirements.txt") %>%
    scan(what="", sep="\n") %>%
    str_extract("[:alpha:]*")
